    ...
    def expand_lt_optimized(var, val):
        ...
        for i in range(width):
            if bitvec[i] == '1':
                ...
                prefix = [f"{'' if bitvec[j] == '1' else '!'}{bits[j]}" for j in range(i)]
    ...
    for match in re.finditer(pattern, precondition):
        ...
        if op == "==":
            ...
            if int(val) == 0:
                ...
                output_clauses.extend([f"!(({clause}))" for clause in expand_lt_optimized(var, str(int(val) + 1))])
...
def Booleanize_precondition(progname, precondition_hl):
    with open(os.path.join(CURRENT_PATH, "candidate_files", progname + ".json"), "r") as file:
        data = json.load(file)
...
def generate_random_cnf(vars, num_clauses, clause_size):
    ...
    for _ in range(num_clauses):
        ...
        clause = [random.choice([v, f'!{v}']) for v in clause]  # Randomly negate some literals
        cnf.append(clause)
    return cnf
...
def random_satisfiable_cnf(variables, number_of_clauses, seed=None):
    ...
    while len(cnf) < number_of_clauses:
        ...
        if not any((assignment[var] if polarity else not assignment[var]) for var, polarity in clause):
...
if __name__ == "__main__":
    ...
    parser.add_argument("--init_states", type=str, help="default = True", default="True", dest="init_states")
    ...
    if progname not in ("Duel", "Prinsys", "BiasDir"):
        with open(os.path.join(CURRENT_PATH, "candidate_files", progname + ".json"), "r") as file:
            ...
            vars_to_expand = {v for v in data_old["Program_variables"]["Vars"] if len(old_varmap.get(v, [])) >= 2}

        
        updated_data = update_varmap(data_old, old_varmap, vars_to_expand, args.num_bits)   
        with open(os.path.join(CURRENT_PATH, "candidate_files", progname + ".json"), "w") as file:
            json.dump(updated_data, file, indent=4)
    
    with open(os.path.join(CURRENT_PATH, "candidate_files", progname + ".json"), "r") as file:
        data = json.load(file)
        loop_guard = data['Loop guard']
        if progname not in ("Temp", "Fair", "BigGeo0", "BigGeo1", "BigGeo2", "BiasDir", "Duel", "GeoAr", "Unif2", "Unif4"):
            loop_guard = Booleanize_precondition(progname, loop_guard)
        
        program_variables = data['Program_variables']['Vars']
        variable_map = data['Program_variables']['Varmap']
        if progname not in ("BigGeo2"):
